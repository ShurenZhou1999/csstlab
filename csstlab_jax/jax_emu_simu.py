...
from .jax_base import *
from .jax_GaussianProcess import ConstantKernel, RBF
...
class Emulator_simu(BaseEmulator_GP):
    ...
    def __set_rescaleDamping(self, k):
        ...
        Damping_r2_1 = lambda paras, x : paras[0] * jnp.exp( - (x-paras[1]) )
        Damping_r2_0 = lambda paras, x : paras[0] * jnp.exp( - (x-paras[1])**2 )
    ...
    def __offset(self, i, j, pk_T, ):
        ...
        if (i, j) in self.CrossZeros_DampFunc:
            offset = jnp.sqrt( pk_T[..., i, i, :] *pk_T[..., j, j, :] )
    ...
    def __rescale_inv(self, i, j, pk_T, ratio=None, ):
        pk_t = pk_T[..., i, j, :]#.reshape(-1, self.Nk)
        offset = self.__offset(i, j, pk_T, )
        return jnp.exp(ratio) * (pk_t+offset) - offset
    ...
    def __call__(self, Param, pk_T, ):
        ...
        '''
        #y_pred = jnp.zeros(( self.Nz , 6, 6, self.Nk, ))
        #for ipk, jpk in self._index:
        #    coeffs = self.GPs[ipk][jpk].predict_JAX( Param )
        #    r_pred = self.PCA_Sim( ipk, jpk, coeffs )
        #    #r_pred[ np.isnan(r_pred) ] = 0
        #    y_pred = y_pred.at[:, ipk, jpk ].set( self.__rescale_inv(ipk, jpk, pk_T, r_pred ) )
        #    y_pred = y_pred.at[:, jpk, ipk ].set( y_pred[:, ipk, jpk ] )
        #return y_pred
        y_pred = self._empty_list
        for ipk, jpk in self._index:
            coeffs = self.GPs[ipk][jpk].predict_JAX( Param )
            r_pred = self.PCA_Sim( ipk, jpk, coeffs )
            #r_pred[ np.isnan(r_pred) ] = 0
        ...
        y_pred = jnp.moveaxis( jnp.array(y_pred), (2, 0, 1, 3), (0, 1, 2, 3) )
        return y_pred
...
class PrincipalComponentAnalysis__simu:
    ...
    def read_PCs(self, ):
        ...
        return jnp.vstack(Rij), jnp.vstack(Aij)
